...
Bei dieser und allen anderen Varianten sind die A-priori-Wahrscheinlichkeiten $P(G_i)$, dass sich hinter dem Tor $i$ das Auto befindet, gleich. Deshalb lassen sich diese beim Satz von Bayes (\autoref{equ:bayes}) kürzen und später kann die vereinfachte Version (\autoref{equ:bayes_simpl}) verwendet werden:
...
\begin{equation}
    \begin{split}
        ...
        P(G_2 | M_3) & = \frac{0}{\frac{1}{2}+1+0} = 0
    \end{split}
\end{equation}

Somit ist bewiesen, dass die Wahrscheinlichkeit von anfangs $\frac{1}{3}$ durch das Wechseln auf Tor 2 auf ein Wahrscheinlichkeit von $\frac{2}{3}$ erhöht werden kann. Dieses Szenario entspricht auch dem klassischen Monty-Hall-Problem, wie es von vos Savos vorgestellt wurde.
